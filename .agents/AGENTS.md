# AI Executive OS — Frontend Agent Base Instructions
...
Supabase). This file is the base floor for every agent session. Read it first,
then comply with every rule below — never write code in a way that violates these
boundaries.

## Golden rules (never break)

1. **Frontend only.** Supabase migrations, database schema, and RLS are owned by
   the **backend repo** — never add them here. See `docs/tenancy/MULTI_TENANCY.md`.
...
3. **State ownership:** *server-owned data → RTK Query* (endpoints under
   ...
   `useVisibilityPolling` for the same endpoint. See `docs/RTK/…`.
...
## Reading order before you build

- `AGENTS.md` (root, this file), `src/README.md`
- `docs/tenancy/MULTI_TENANCY.md`, `docs/RTK/…`
- Relevant feature under `src/<module>/` and its existing hooks/slices/endpoints
...
This `.agents/skills/` folder carries two resources:
- **Skills** (`architect`, `recover`, `remember`, `review`, `imprint`) —
  repo-specific runbooks. Invoke with `/architect`, `/recover`, `/remember`,
  `/review`, `/imprint`.
- Use `/remember save` at the end of every session and `/remember restore` at the
  start of the next so nothing is lost between sessions.
