...
} from "@/knowledge/types/integrations";
import { parseDepartmentScope } from "@/knowledge/utils/parseDepartmentScope";
import { DEFAULT_INTEGRATION_VALUES } from "@/knowledge/utils/IntegrationValues";
...
export function useIntegrations({ onSynced }: UseIntegrationsProps = {}) {
  ...
  const busy =
    isSavingIntegration || isSyncingNotion || isSyncingDrive || isResyncingAll;
  ...
  const runNotionSync = useCallback(
    async (data: IntegrationFormData) => {
      ...
      try {
        await syncNotionPage({
          ...
          allowedDepartments: parseDepartmentScope(data.deptScope || ""),
        }).unwrap();
